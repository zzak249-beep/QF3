"""
╔══════════════════════════════════════════════════════════════════╗
║         CRYPTO SCANNER v4.1 — FIXES AUTO-TRADE                 ║
║                                                                  ║
║  FIXES v4.1:                                                     ║
║  ✅ FIX CRÍTICO: Auto-trade ahora abre también en STD            ║
║  ✅ FIX: redondear_qty usa step_size real del instrumento        ║
║  ✅ FIX: get_info_instrumento cachea resultados (sin spam API)   ║
║  ✅ FIX: BTC y pares caros usan step_size correcto (0.00001)    ║
║  ✅ NUEVO: LOG detallado de por qué no se abre un trade          ║
║  ✅ NUEVO: STD_AUTOTRADE env var (default true)                  ║
║  ✅ NUEVO: MIN_NOTIONAL check antes de enviar orden              ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
_estado = {
    ...
    "balance": 0.0, "pnl_dia": 0.0, "version": "4.1",
    "circuit_breaker": False, "modo": "iniciando",
    "ultimo_skip": ""   # ← NUEVO: último motivo de skip de trade
}
...
MIN_NOTIONAL     = float(os.getenv("MIN_NOTIONAL", "2.0"))  # USDT mínimo notional real
...
# ✅ NUEVO: controla si STD también abre trades (antes solo FUEL/SUP)
...
# ✅ NUEVO: caché de info de instrumentos (evita llamadas repetidas)
_info_cache: dict = {}
...
log = logging.getLogger("ScannerV41")
...
def get_info_instrumento(simbolo: str) -> dict:
    """✅ MEJORADO: cachea resultados para no llamar la API en cada trade."""
...
def redondear_qty(qty: float, step: float) -> float:
    if step <= 0:
        return round(qty, 4)
    # Usar floor con precisión adecuada al step
...
# ══════════════════════════════════════════════════════════════════════
# MOTOR QF×JP — ANÁLISIS (sin cambios respecto a v4.0)
# ══════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════
# AUTO-TRADE — ✅ MEJORADO con logs de skip detallados
# ══════════════════════════════════════════════════════════════════════
...
def _skip(razon: str):
    """Registra el motivo de skip en el estado del health endpoint."""
...
def abrir_trade(simbolo: str, precio: float, direccion: str) -> Optional[dict]:
    ...
    if not AUTO_TRADE and not DRY_RUN:
        _skip("AUTO_TRADE=false y DRY_RUN=false")
        return None
    if simbolo in trades_abiertos:
        _skip(f"{simbolo} ya tiene trade abierto")
        return None
    if circuit_breaker_activo():
        _skip("circuit breaker activo")
        return None
    if simbolo in BLACKLIST:
        _skip(f"{simbolo} en blacklist")
        return None
    ...
    if balance < usdt_trade:
        _skip(f"balance ${balance:.2f} < trade ${usdt_trade:.2f}")
        return None
    ...
    if qty < info["min_qty"]:
        _skip(f"{simbolo} qty {qty} < min_qty {info['min_qty']} "
              f"(necesitas ≥ ${info['min_qty'] * precio / LEVERAGE:.2f} USDT de trade)")
        return None

    if notional < MIN_NOTIONAL:
        _skip(f"{simbolo} notional ${notional:.2f} < MIN_NOTIONAL ${MIN_NOTIONAL}")
        return None
    ...
    if not orden:
        _skip(f"{simbolo} orden MARKET fallida")
        return None
...
def construir_alerta(par: dict) -> str:
    ...
    if AUTO_TRADE:
        if es_sup or es_fuel or STD_AUTOTRADE:
            ...
            lineas.append("Auto-trade: STD_AUTOTRADE=false — solo alertas para STD")
    else:
...
def construir_resumen(resultados: list, btc_cambio: float, intervalo: int) -> str:
    ...
    skip_str = f"\nÚltimo skip: {_estado['ultimo_skip']}" if _estado["ultimo_skip"] else ""
    ...
    lineas = [
        f"QF×JP v4.1{modo_str} — {ahora}",
        f"BTC {signo}{btc_cambio:.2f}% | próximo scan {intervalo//60}min",
        f"W/L: {wr_str} | PnL día: ${pnl_acumulado_dia:+.2f} | Racha: {racha_perdidas}{cb_str}{skip_str}",
        f"{'─'*24}",
    ]
...
# ══════════════════════════════════════════════════════════════════════
# LOOP PRINCIPAL — ✅ MEJORADO: STD también dispara auto-trade
# ══════════════════════════════════════════════════════════════════════
def ejecutar():
    ...
    log.info(f"╔══════════════════════════════════╗")
    log.info(f"║  QF×JP Scanner v4.1 — {modo:<10} ║")
    log.info(f"╚══════════════════════════════════╝")
    ...
    log.info(f"  CB: {CB_MAX_LOSSES} pérdidas → pausa {CB_PAUSA_MIN}min")
    ...
    enviar_telegram(
        f"🤖 *QF×JP Scanner v4.1 iniciado*\n"
        f"Modo: *{modo}*\n"
        f"Config: ${TRADE_USDT} × {LEVERAGE}x | SL {SL_PCT}% | TP {TP_PCT}%\n"
        f"STD auto-trade: {'✅' if STD_AUTOTRADE else '❌'} | "
        f"Max trades: {MAX_TRADES}\n"
        f"CB: {CB_MAX_LOSSES} pérd → {CB_PAUSA_MIN}min\n"
    ...
    while True:
        ...
        try:
            ...
            for par in resultados:
                ...
                # ✅ FIX PRINCIPAL: STD también puede abrir trade si STD_AUTOTRADE=true
                es_sup_fuel = par["long_sup"] or par["short_sup"] or \
                              par["long_fuel"] or par["short_fuel"]
                es_std      = (par["long_std"] and not par["long_fuel"]) or \
                              (par["short_std"] and not par["short_fuel"])
                accionable  = es_sup_fuel or (es_std and STD_AUTOTRADE)

                if not accionable:
                    continue

                # Cooldown de alertas (no de trades)
                if time.time() - alertas_enviadas.get(sym, 0) < ALERTA_COOLDOWN:
                    pass  # no re-enviar alerta pero sí intentar trade si no está abierto
                else:
