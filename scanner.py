    ...
    try:
        ...
        for sym in cerrados:
            ...
            if k:
                ...
                if ganado:
                    ...
                    if racha_perdidas >= CB_MAX_LOSSES:
                        ...
                        enviar_telegram(
                            ...
                            + str(racha_perdidas) + " perdidas -> pausa " + str(CB_PAUSA_MIN) + "min")
                ...
                enviar_telegram(
                    "*Trade cerrado*: " + sym.replace('-USDT', '') + "\n"
                    + "Dir: " + str(trade['direccion']) + " | Entrada: `" + str(en) + "`\n"
                    + "Cierre: `" + f"{pa:.6f}" + "`\n"
                    + resultado + "\n"
                    + "PnL: `$" + f"{pnl_usdt:+.2f}" + "` | Dia: `$" + f"{pnl_acumulado_dia:+.2f}" + "`"
                )
...
def construir_alerta(par: dict) -> str:
    ...
    barra = "X" * db + "." * (8 - db)
    trailing = ("  Trailing SL: `" + str(TRAILING_PCT) + "%`\n") if TRAILING_PCT > 0 else ""
...
def construir_resumen(resultados: list, btc_cambio: float, intervalo: int) -> str:
    ...
    if time.time() < circuit_breaker_hasta:
        ...
        cb_str = "\nCircuit breaker: " + str(rest) + "min restantes"
...
def ejecutar():
    ...
    while True:
        ...
        try:
            ...
            enviar_telegram("*Error en scanner*\n`" + str(e)[:200] + "`")
            intervalo = INT_NORMAL
