"""
╔══════════════════════════════════════════════════════════════════╗
║         QF×JP Bot v6.0 — FUSION COMPLETA                        ║
║                                                                  ║
║  BASE: Bot Async v5.7 (arquitectura superior)                    ║
║  + MEJORAS del Scanner v4.1:                                     ║
║                                                                  ║
║  ✅ Circuit breaker (N pérdidas consecutivas → pausa)            ║
║  ✅ Cooldown por símbolo tras pérdida                            ║
║  ✅ Resumen diario P&L automático a medianoche UTC               ║
║  ✅ Log CSV de todos los trades                                   ║
║  ✅ Health endpoint JSON enriquecido (/json)                     ║
║  ✅ DRY_RUN mode (simula sin operar)                             ║
║  ✅ STD_AUTOTRADE env var                                        ║
║  ✅ Alertas Telegram con dashboard completo QF×JP                ║
║  ✅ ultimo_skip visible en health endpoint                       ║
║  ✅ BalanceCache centralizado (sin rate limiting)                ║
║  ✅ Trailing SL dinámico por ATR                                 ║
║  ✅ Multi-TF: 1m+3m+15m+1h + OFI+FR+OI                         ║
║  ✅ RiskManager DD diario + PerformanceTracker                   ║
║  ✅ SessionFilter NY/LDN                                         ║
║  ✅ Maker orders (fee 0.04%)                                     ║
...
# ESTADO GLOBAL
# ══════════════════════════════════════════════════════════════════════
active_positions: dict            = {}   # sym → posición activa
prev_oi:          dict[str,float] = {}
cooldown_sym:     dict[str,float] = {}   # sym → timestamp fin cooldown
...
async def _registrar_cierre(sym, side, entry, price, pnl_pct,
    ...
    _log_csv(sym, side, entry, price, pnl_pct, pnl_usdt,
             ganado, tier, conv, trail_active, DRY_RUN)
    ...
    if ganado:
        ...
        log.info(f"Cooldown {sym}: {COOLDOWN_LOSS_MIN}min por pérdida")
        emoji = "❌"
    ...
    log.info(f"Cerrado: {sym} {side} | {resultado} | ${pnl_usdt:+.2f} USDT")
        ...
        try:
            ...
            if pos:
                ...
                if reason:
                    ...
                    await bal_cache.get(force=True)  # refresco tras cierre
            ...
            if symbol not in active_positions and sig["direction"]:
                ...
                # Alerta Telegram enriquecida
                await _enviar_alerta_entrada(tg, symbol, sig, price, size,
...
# ══════════════════════════════════════════════════════════════════════
# ALERTA TELEGRAM DE ENTRADA (dashboard completo)
# ══════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════
# STATUS LOOP (horario + reset diario)
# ══════════════════════════════════════════════════════════════════════
async def status_loop(tg, bal_cache: BalanceCache):
    ...
    while not _stop_event.is_set():
        ...
        try:
            ...
            try:
                from performance import PerformanceTracker  # avoid circular
            except Exception:
                pass
...
async def main():
    ...
    log.info("═══════════════════════════════════════════")
    log.info("  QF×JP Bot v6.0  |  BingX Futures")
